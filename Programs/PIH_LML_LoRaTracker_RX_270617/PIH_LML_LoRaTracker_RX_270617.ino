...
#define defaultfont 0               //default font, 0 for digole displays 6 for Nokia 5110 backpack

#include <Arduino.h>
#include "Tracker_Definitions.h"
#include <SoftwareSerial.h>

SoftwareSerial ss(GPSRX, DisplayTX); //The serial connection to the GPS device
...
void setup()
{
  ...
  {
    Serial.println(F("RFM98 Error"));
    display_Setfont(0);
    display_SetCurPos(0, 5);
    display_Text("RFM98 Error");
    systemerror();
  }
  ...
  ss.begin(GPSBaud);                           //startup soft serial for GPS and Digole display
  display_Cls();
  display_Setfont(defaultfont);
  display_SetCurPos(0, 5);
  display_Text("RX ");
