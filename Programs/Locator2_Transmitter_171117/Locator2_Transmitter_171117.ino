//Locator2_Transmitter_171117.ino
//**************************************************************************************************
// Note:
//
...
#define programname "Locator2_Transmitter_171117"
#define aurthorname "Stuart Robinson"

#include <Arduino.h>
#include <avr/pgmspace.h>

#include "Locator2_Transmitter_Settings.h"
...
**************************************************************************************************
  ...
  Check if Use_GPS_SoftwareBackup is working
...
unsigned int pulseerrorcount = 50;                    //number of RC pulse errors recorded, start at 50
...
void checkRCpulse()
{
  ...
  boolean width_error = true;
  boolean hold_error = true;
  
  Serial.print(F("Current pulseerrorcount "));
  Serial.println(pulseerrorcount);
  RCPulseLen = pulseIn(RCPulse, HIGH);
  Serial.print(F("RC Servo Pulse "));
  Serial.print(RCPulseLen);
  Serial.println(F("uS"));

  if ( (RCPulseLen < RCpulseshort) ||  (RCPulseLen > RCpulselong) )
  {
    lora_Tone(1500, 100, 10);                               //transmit an short high frequency pip to flag RC pulse error
  }
  else
  {
    width_error = false;
  ...
  {
    Serial.println(F("RC Pulse Hold !"));
    lora_Tone(1500, 125, 10);                               //transmit an short high frequency pip to flag RC pulse error
  }

  if ( width_error | hold_error)
  {
  pulseerrorcount =  pulseerrorcount + inc_on_pulse_error; 
  }
  else
  {
  pulseerrorcount =  pulseerrorcount - dec_on_pulse_OK; 
  }
...
void setup()
{
  ...
  if (isLost())                                  //after reset is lost mode still set ?
  {
    ...
    //sleepSecs(lostclear_CheckmS);
    delay(lostclear_CheckmS);
