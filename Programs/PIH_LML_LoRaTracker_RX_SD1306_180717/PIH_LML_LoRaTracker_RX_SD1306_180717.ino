#define programname "PIH_LML_LoRaTracker_RX_SD1306"
#define programversion "V1.5"
...
Changes:
270617 - Added definitions for Version 4 of DRF1278F tracker
180717 - New program to support SD1306 OLED.
*/

#define GPS                         //comment out if no local GPS is used.
...
#define SCREEN_SMALL_TEXT           //select small screen text, displays more information
//#define SCREEN_LARGE_TEXT           //select large screen text, displays limited information
...
void processGPS()
{
  ...
  if (gps.location.isUpdated() && gps.altitude.isUpdated())       //check the fix is updated
  {
    ...
    Display_SetTextSize(1);
    Display_SetCursor(0, 7);
    disp.print("RX GPS FIX           ");                          //make sure entire line is cleared
...
void display_Update()
{
  #ifdef SCREEN_SMALL_TEXT 
  updatescreen0();
  #endif

  #ifdef SCREEN_LARGE_TEXT 
  updatescreen1();
  #endif
}
...
void setup()
{
  ...
  Display_Setup();                             //sets up instance of display called disp 
  
  Display_Clear();
  Display_SetTextSize(1);
  Display_SetCursor(0, 7);
  disp.print("RX ");
  disp.print(GetSupplyVolts());
  disp.print("mV            ");
  delay(2000);
